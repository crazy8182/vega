...
async def start(client, message):
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[
                        ...
                        InlineKeyboardButton('😇Bot owner😇', url="https://t.me/Mahwsh8182")
                   ], [
            ...
            msg = await client.send_cached_media(
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                            [
                                InlineKeyboardButton('👨‍💻Sᴜᴘᴘᴏʀᴛ Gʀᴏᴜᴘ', url=f'https://t.me/{SUPPORT_CHAT}'),
                                InlineKeyboardButton('🗃️🎥Movie group', url="https://t.me/+A4tVZCXbK2s2YzJl")
                            ],[
                                InlineKeyboardButton("😇Bot owner😇", url="t.me/Mahesh8182"),
                                InlineKeyboardButton("Uᴘᴅᴀᴛᴇs Cʜᴀɴɴᴇʟ", url=CHNL_LNK)
                            ],[
                                InlineKeyboardButton('Movie Cʜᴀɴɴᴇʟ', url=CHNL2_LNK)
                            ],[   
    ...
    files = files_[0]
    title = '@vegamoviesnewin  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))
    ...
    if CUSTOM_FILE_CAPTION:
        try:
            ...
            f_caption=f_caption
    if f_caption is None:
        f_caption = f"@vegamoviesnewin  {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))}"
    if not await check_verification(client, message.from_user.id) and VERIFY == True:
        btn = [[
            InlineKeyboardButton("Verify", url=await get_token(client, message.from_user.id, f"https://telegram.me/{temp.U_NAME}?start="))
        ]]
